  ...
  <div class="kb-shell" :class="{ 'ai-open': aiOpen }">
    <header class="kb-header">
      ...
      <nav class="top-nav">
        ...
        <button v-if="can('user:update') || can('role:assign')" :class="{ active: viewMode === 'admin' }" @click="viewMode = 'admin'">管理</button>
      </nav>

      <div class="header-actions">
        <button v-if="session && can('ai:use')" class="ai-toggle" :class="{ active: aiOpen }" @click="aiOpen = !aiOpen">
          AI 面板
        </button>
      ...
      <main class="workspace">
        ...
        <section v-else class="doc-view">
          <div class="doc-toolbar">
            ...
            <div class="toolbar-actions" v-if="activeArticle">
              <button v-if="can('kb:update')" @click="editing = !editing">{{ editing ? '预览' : '编辑' }}</button>
          ...
          <textarea v-if="editing && activeArticle" v-model="editorContent" class="editor" spellcheck="false" />
          <article v-else-if="activeArticle" class="markdown-body" v-html="renderMarkdown(activeArticle.content)" />
      ...
      <aside v-if="aiOpen && session && can('ai:use')" class="ai-panel">
        ...
        <div class="messages">
          <div v-for="(message, index) in messages" :key="index" class="message" :class="message.role">
            ...
            <div class="message-content markdown-body compact" v-html="renderMarkdown(message.content)" />
            <details v-if="message.toolCalls?.length" class="tool-chain" open>
              <summary>工具调用链</summary>
              <ol>
                <li v-for="tool in message.toolCalls" :key="`${tool.name}-${tool.detail}`" :class="tool.status">
            ...
            <div v-if="message.draft" class="draft-box">
              ...
              <button v-if="can('ai:write_kb')" class="primary" @click="applyDraft(message.draft)">确认写入</button>
            </div>
            <div v-if="message.sources?.length" class="source-list">
              <a v-for="source in message.sources" :key="source.url || source.title" :href="source.url" target="_blank" rel="noreferrer">
...
async function removeArticle() {
  if (!activeArticle.value) return
  const confirmed = window.confirm(`确认删除 ${activeArticle.value.path}？该操作会直接删除 Markdown 文件。`)
  if (!confirmed) return
  ...
  while (true) {
    ...
    for (const packet of packets) {
      ...
      if (event === 'meta') {
        ...
        upsertToolStep(assistantMessage, { name: '流式响应', status: 'error', detail: data.message })
...
async function createUser() {
  ...
  if (!username) return
  const readerRole = roles.value.find(role => role.permissions.includes('kb:view')) || roles.value[0]
