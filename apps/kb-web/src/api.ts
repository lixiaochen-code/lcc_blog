...
export const api = {
  ...
  createArticle: (path: string, content: string) =>
    request<Article>('/api/kb/article', { method: 'POST', body: JSON.stringify({ path, content }) }),
  ...
  deleteArticle: (path: string) =>
    request<{ path: string }>(`/api/kb/article?path=${encodeURIComponent(path)}`, { method: 'DELETE' }),
  chat: (payload: { message: string; history: { role: string; content: string }[]; currentPath?: string; conversationId?: string; useWebSearch?: boolean }) =>
    request<{ conversationId: string; content: string; reasoning?: string[]; draft?: Draft; sources?: SearchSource[] }>('/api/ai/chat', {
      method: 'POST',
