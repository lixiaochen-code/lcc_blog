        ...
        >
          <template #item="{ item }">
            <view class="bg-white rounded-[16rpx] p-[24rpx] mb-[20rpx]" @click="handleDetail(item)">
              <!-- 订单编号 + 状态 -->
              <view class="flex justify-between items-center mb-[6rpx]">
              ...
              <view class="text-[26rpx] text-[#333] mb-[12rpx]">
                <text>店铺名称:</text>
                <text>{{ (item as unknown as OrderListItem)?.storeInfo?.name }}</text>
              </view>

              <!-- 服务信息 -->
              <view class="flex gap-[20rpx] mb-[24rpx]" @click.stop="handleToDetails(item)">
                <image
...
<script setup lang="ts">
  ...
  // ========== Tab 配置: 待支付 / 服务中 / 已完成 ==========
  const tabList = ref([{ name: '待支付' }, { name: '服务中' }, { name: '已完成' }])
  const currentTab = ref(0)

  // Tab 对应的查询参数
  const TAB_QUERY_MAP: Record<number, Record<string, unknown>> = {
    0: { orderStatus: '101,104' },
    1: { orderStatus: '201,301' },
    2: { orderStatus: '302,401,402' },
  }
  ...
  // ========== 请求适配（LoadingMore 传入 page/limit，需映射为 pageNumber/pageSize） ==========
  ...
  const handleToDetails = (item: unknown) => {
    const orderItem = item as OrderListItem
    uni.navigateTo({
      url: `/pages_client/pages/service/details/index?id=${orderItem.storeServicesId}`,
    })
  }
</script>
