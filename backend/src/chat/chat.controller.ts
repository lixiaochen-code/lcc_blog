...
export class ChatController {
  ...
  private async authenticateActor(authorization: string | undefined) {
    ...
    const actor = (await this.authService.authenticateByToken(
      token,
    )) as ChatActor;
  ...
  async chat(
    ...
    const { plannerModel, plan, plannerDurationMs } = await this.resolvePlan(
      actor,
      message,
      history,
    );
    ...
    if (plan.action === "none") {
      ...
      return this.buildFinalPayload({
        ...
        plannerError: plannerError || undefined,
        assistantMessage:
          plan.reply || "这次不需要执行知识库动作，我先直接回复你。",
      });
    }

    const permissionRequired = this.chatService.getActionPermission(
      plan.action,
    );
    ...
    if (execution.ok) {
      ...
      assistantMessage = this.buildExecutionFailureMessage(
        plan,
        executionPayload,
        execution.stderr,
      );
